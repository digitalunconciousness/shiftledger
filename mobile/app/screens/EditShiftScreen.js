...
export default function EditShiftScreen({ route, navigation }) {
  ...
  const { updateShift, jobs, fetchJobs } = useShiftStore();

  const [date, setDate] = useState(shift.date);
  const [hourlyRate, setHourlyRate] = useState(String(shift.hourly_rate));
  const [hoursWorked, setHoursWorked] = useState(String(shift.hours_worked));
  const [tipMode, setTipMode] = useState(shift.tip_mode);
  const [tipInput, setTipInput] = useState(String(shift.tip_input));
  const [notes, setNotes] = useState(shift.notes || '');
  const [jobId, setJobId] = useState(shift.job_id || null);
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        <Text style={styles.sectionTitle}>Edit Shift</Text>
